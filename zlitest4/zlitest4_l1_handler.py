...
# from zlitest4_tl1_connection import Zlitest4TL1Connection
from zlitest4_cli_connection import Zlitest4CliConnection, Zlitest4DefaultCommandMode, Zlitest4EnableCommandMode, Zlitest4ConfigCommandMode

class Zlitest4L1Handler(L1HandlerBase):
    ...
    def __init__(self, logger):
        ...
        self._logger.info('Constructor enter')

        try:
            with open(os.path.join(os.path.dirname(sys.argv[0]), 'zlitest4_runtime_configuration.json')) as f:
                o = json.loads(f.read())
        ...
        Zlitest4DefaultCommandMode.PROMPT_REGEX = o.get("common_variable", {}).get("default_prompt", r'>\s*$')
        Zlitest4EnableCommandMode.PROMPT_REGEX = o.get("common_variable", {}).get("enable_prompt", r'#\s*$')
        Zlitest4ConfigCommandMode.PROMPT_REGEX = o.get("common_variable", {}).get("config_prompt", r'[(]config.*[)]#\s*$')

        self._example_driver_setting = o.get("driver_variable", {}).get("example_driver_setting", False)
        self._logger.info('Constructor exit')
    ...
    def login(self, address, username, password):
        ...
        # self._tl1_connection = Zlitest4TL1Connection(self._logger, self._host, self._port, self._username, self._password)
        self._cli_connection = Zlitest4CliConnection(self._logger, 'ssh', self._host, self._port, self._username, self._password)
    ...
    def get_resource_description(self, address):
        ...
        SWITCH_MODEL = 'Zlitest4'
        BLADE_PREFIX = 'module'
        BLADE_FAMILY = 'L1 Switch Blade'
        BLADE_MODEL = 'Zlitest4 Blade'
        PORT_PREFIX = 'port'
        PORT_FAMILY = 'L1 Switch Port'
        PORT_MODEL = 'Zlitest4 Port'
