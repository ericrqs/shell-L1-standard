# cloudshell-L1-Zlitest7

## Installation
Copy to c:\Program Files (x86)\QualiSystems\CloudShell\Server\Drivers on CloudShell machine:
- Zlitest7.exe (If the EXE was downloaded directly from the web, be sure to right click the EXE, open Properties, and *UNBLOCK* the file)
- zlitest7_runtime_configuration.json

To customize the port, prompt, and other driver-specific settings, edit:
- zlitest7_runtime_configuration.json

Import zlitest7_datamodel.xml into Resource Manager
...
- Connect the route
- See log files in c:\Program Files (x86)\QualiSystems\CloudShell\Server\Logs\\Zlitest7_*\
...
- In Git Bash:
  - git clone https://github.com/QualiSystems/cloudshell-L1-Zlitest7.git


### What to edit
- zlitest7_l1_handler.py
  ...
  - Load all settings from c:\Program Files (x86)\QualiSystems\CloudShell\Server\Drivers\\zlitest7_runtime_configuration.json
  - Handle all communication with the device
    - e.g. paramiko
    - e.g. requests
- zlitest7_datamodel.xml
...
The only .py you should need to edit is zlitest7_l1_handler.py.

The address, username, and password of the switch resource become known to the driver only when 'login' is called. 

The port number is not stored on the resource. If it can't just be hard coded in the driver, take the setting from c:\Program Files (x86)\QualiSystems\CloudShell\Server\Drivers\\zlitest7_runtime_configuration.json.
...
zlitest7_runtime_configuration.json is not mandatory. If your driver doesn't need any runtime settings, you can delete all the code in zlitest7_l1_handler.py that deals with the JSON.
